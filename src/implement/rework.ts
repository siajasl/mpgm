...
export function renderReview(request: {
  ...
  if (request.undeclared.length > 0) {
    lines.push(
      ...
      'counts: saying it in your commit message, in a comment, in `summary`, or',
      'by arguing it in a design document or a contract leaves the deviation',
      'undeclared and refuses the merge exactly as saying nothing would.',
      '',
      'Fixing a departure does not stop the reviewer reporting it either. The',
      'reviewer reports what it finds the change departing from, and is not told',
      'what you declared, so a departure you fixed and did not declare can still',
      'refuse the merge. If you fix one, declare it as well.',
      '',
