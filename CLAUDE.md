...
`npm run check` runs the whole CI pipeline locally — format, lint, typecheck, build, test, secret scan, milestone demos — and is what `main` must pass.

**While iterating, run `npm run check:fast`** — the same pipeline without the milestone demos. It is 26s against 94s, because the deploy demos (`demo:env`, `demo:release`, `demo:verify`, `demo:gate`) shell to real Docker and are 55% of the full run's wall clock. Almost nothing you change mid-task affects them, and a session has a bounded number of steps: T4.1.4's first attempt spent 80 of its 119 tool calls in Bash and ran out of turns having written 205 lines. Run the full `check` before you commit, and whenever you have touched `deploy/`, `src/env/`, `src/release/` or a demo script.

Narrower still: `npm run lint`, `npm run typecheck`, `npm test` (`npm run test:watch` while iterating, `npm run test:coverage` for coverage), `npm run build`. A single test file: `npx vitest run src/path/to/file.test.ts`.

CI is split into one job per merge-check kind (`build`, `lint`, `typecheck`, `test`, `scan`) because the merge gate decides per kind — see below. Build precedes test: crash, worktree and secret-scan fixtures run as subprocesses against `dist/`, because Node strips TS types but will not resolve a `.js` specifier to a `.ts` file. Milestone verification demos live in `scripts/demo/`. `demo:crash` (M1.1), `demo:cli` (T1.3.6), `demo:trace` (T2.2.5), `demo:ingest` (T2.2.7), `demo:switchover` (T3.1.8), `demo:env` (T4.1.1), `demo:release` (T4.1.2), `demo:verify` (T4.1.3) and `demo:gate` (T4.1.4a) are offline and run in CI; `demo:agent` (M1.2), `demo:definition` (M1.3), `demo:scope` (T2.1.2), `demo:design` (T2.1.3), `demo:plan` (T2.2.3) and `demo:test-phase` (M3.2, T4.3.5) make real model calls, so they are operator-run and need credentials — it is deliberately not in CI, since a verification that silently skipped itself would be worse than none. `demo:env`, `demo:release`, `demo:verify` and `demo:gate` need a Docker daemon rather than credentials, which `ubuntu-latest` ships preinstalled — they shell to real `docker compose`/`docker build` against the committed IaC and sample service rather than a mocked provider, the only way "an environment comes up from repository config alone", "a rollback path is tested", "an induced regression auto-rolls back" and "a gated environment refuses delivery without a confirmation" are actually shown rather than asserted. `demo:test-phase` closes M3.2 on a run rather than on its tasks, and its run has a standing finding: it verifies TST-3, TST-4 and TST-5 over a real subject and **cannot** report TST-2's requirement coverage, so REQUIREMENTS' first Test-gate clause (all Must-have requirements verified) is not met by the Test phase as built — `requirementCoverageReport` needs a quarantine ledger and the whole Scope list no `nfr` node supplies (`phases/test.yaml`'s gate description, DESIGN §9 decision 15). The demo prints that gap with the Must-have requirements it leaves uncovered rather than weakening the gate to close the milestone. A second finding stands against T4.3.5's own record, and belongs here because this is where the figure it corrupts is read: commit `627e6cd` carries `Verifies: TST-1, TST-2, TST-5`, and TST-1 and TST-2 are untrue by that change's own account. `6b80e01` retracted them in the log, which is all the log can do — `Verifies:` is the only trailer `TraceIndexStore.coverage()` counts, its links are keyed by the source commit that wrote them, and nothing reads a later commit as withdrawing an earlier claim (`verified` is `verifiedBy.length > 0`, `src/trace/index-store.ts`). So mpgm's own coverage report reads TST-1 and TST-2 as **verified** from that commit, and will until a retraction mechanism exists; there is none, and T4.3.12 is filed for it. Until it lands, treat those two rows as spurious — and note that the same hole makes every other `Verifies:` claim in this history unwithdrawable, not just these two. A third row is spurious for the same reason and is named here rather than only in a commit body: `264d947` (T4.3.7) carries `Verifies: PLN-4, HIL-5, NFR-3`, and **NFR-3 and PLN-4 are untrue of it**. NFR-3 is the 10%-of-wall-clock harness-overhead threshold; that change measures nothing against it — its `state/overhead.test.ts` cases check `computeHarnessOverhead`'s settled population over a synthetic fixture, which is arithmetic about a new task status, not the requirement. PLN-4 is incremental replanning; `supersede` records a retirement *after* a replan somebody did by hand, and checks no clause of it. Only HIL-5 (operator interventions recorded in the audit log) is checked there, by the assertion that the appended `TaskSuperseded` carries who decided it and why. The claim cannot be withdrawn — same mechanism, same T4.3.12 — so read NFR-3 and PLN-4 as **unverified by this history** whatever `coverage()` says.
...
- Trailers the trace index reads (ADR-4, `src/trace/links.ts`), comma-separated ids, one or more per commit (CONV-8): `Traces:` / `Traces-To:` / `Implements:` / `Closes-Task:` all become a traces-to link; `Verifies:` is the only one TST-2 coverage counts, so it is the one to write when the commit is what checks a requirement rather than merely serving it (CONV-9). A value that is not id-shaped (a section number, a milestone's verification prose) is reported, not indexed — write it if it is true, it just will not resolve to a node. Any other trailer key is not read at all. A trailer only counts if it sits in a paragraph of its own where *every* line is `Key: value` shaped, set apart from prose by a blank line above (T4.2.11, CONV-8) — put trailers together, the way `Traces:` sits above `Co-Authored-By:` in this history. A `Key: value` line that opens a wrapped sentence inside a prose paragraph is not read as a claim, however true the sentence is.
