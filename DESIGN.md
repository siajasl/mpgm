...
The kernel and CLI are TypeScript. **Why:** the Claude Agent SDK, Claude Code hook/plugin surface, and the MCP ecosystem are TypeScript-first; zod gives runtime schema validation matching AGT-3. **Trade-off:** Python's eval/data tooling is stronger; accepted since evals (AGT-6) are driven through the SDK, not notebooks. **Revisit if:** eval sophistication outgrows the SDK harness.
